...
- 가입 없이 빈 냉장고를 바로 체험하는 게스트 진입을 추가했다.
- 게스트도 서버 HttpOnly 세션과 사용자 ID로 재고가 분리되며, 이용 종료 시 임시 계정과 냉장고 기록을 삭제한다.
- 게스트에는 연락처·비밀번호 관리 화면을 노출하지 않고, 내부 임시 식별자도 API 응답에서 숨긴다.
- 서버 회귀 테스트 87개, 타입 검사와 lint를 통과했다. 실제 AI 호출과 운영 AI 설정 변경은 없다.
...
- 생활형 냉장고 UI로 전면 정리: off-white/teal, 영수증 홍보·대형 브리핑·이모지·카드 과잉 제거.
- 홈 보관 요약/가까운 D-Day 목록과 두 추가 경로, 4개 재고 탭과 별도 곧 소비 필터.
