...
export const users = sqliteTable('users', {
  ...
  phone: text('phone').notNull(), passwordHash: text('password_hash').notNull(),
...
export const sessions = sqliteTable('sessions', {
  ...
  createdAt: text('created_at').notNull(), lastUsedAt: text('last_used_at').notNull(),
...
export const userInventories = sqliteTable('user_inventories', {
  ...
  snapshot: text('snapshot').notNull(), revision: integer('revision').notNull().default(0),
  createdAt: text('created_at').notNull(), updatedAt: text('updated_at').notNull(),
});

export const loginAttempts = sqliteTable('login_attempts', {
  keyHash: text('key_hash').primaryKey(), failures: integer('failures').notNull().default(0),
  blockedUntil: text('blocked_until'), updatedAt: text('updated_at').notNull(),
});
