...
## 사용자 흐름
회원가입/로그인 또는 게스트로 이용하기 → 사용자별 빈 냉장고 → 구매 추가(영수증/온라인 캡처/직접 입력) → 설정에 따른 Mock/OpenAI 분석 → 이름·수량·단위·구매일·보관 확인/수정 → 등록 → 홈 우선순위 → 자연어 명령 확인 → 재고와 거래 원장 동시 갱신.
...
게스트는 POST /api/auth의 guest 작업으로 임시 계정과 세션을 발급받는다. 같은 inventory/AI 소유권 경계를 사용하며 다른 게스트·회원 데이터에 접근할 수 없다. 자동 로그인과 비밀번호 변경은 제공하지 않는다. 이용 종료 시 임시 계정과 냉장고 snapshot을 삭제한다.
